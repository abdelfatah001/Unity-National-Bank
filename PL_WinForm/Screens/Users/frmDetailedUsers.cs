...
{
    public partial class frmDetailedUsers : Form, IDetailedUserScreen
    {
        public frmDetailedUsers(clsUser user, short index)
        {
            InitializeComponent();
            ...
            ((IDetailedUserScreen)this).ReintializeCtrl(user);
        }
        ...
        {
            if (ctrlDetailedUsers1.UpdateService.DataChanged)
                DataBack?.Invoke(this, _indexInMenu, ctrlDetailedUsers1.SelectedRecord);    
        }

        private void frmDetailedUsers_FormClosed(object sender, FormClosedEventArgs e)
        {
            IsOpened = false;
        }
    }
